...
data Action
  = DecAngVelocity Axis
  | IncAngVelocity Axis
  | IncCubeVelocity Axis
  | DecCubeVelocity Axis
  | ReverseCube Axis
...
cubes =
    ...
    where
        ...
        handleAction query = case query of
            ...
            IncCubeVelocity axis -> do
                    cube <- H.get
                    let {xa, ya, za} = cube.angVel
                    _<- H.modify (\c ->
                         case axis of
                           X -> c { angVel { xa = xa + accelerateBy*3.0 } }
                           Y -> c { angVel { ya = ya + accelerateBy*3.0 } }
                           Z -> c { angVel { za = za + accelerateBy*3.0 } }
                        )
                    pure unit

            DecCubeVelocity axis -> do
                    cube <- H.get
                    let {xa, ya, za} = cube.angVel
                    _<- H.modify (\c ->
                         case axis of
...
renderView state = let
    ...
    in
        ...
        , renderButton "vel++" (IncCubeVelocity X)
        , renderButton "vel--" (DecCubeVelocity X)
